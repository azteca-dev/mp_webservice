	...
	*/
	...
	def republica(def json, def params){
		...
		if(json?.by.equals("user")){
			...
			resultCriteria.each{
				vehicleId = PublicaService.existVehicle(it.tech.mxpid)
				if (vehicleId.status != 404) {
					...
					resultFinal.listado.add("404 - Car with carID: ${it.tech.mxpid} not_found")
				}
			}
		}
		...
		if(json?.by.equals("array")){
			...
			json?.array.each{
				...
				vehicleId = PublicaService.existVehicle(resultArray.tech.mxpid)
				if (vehicleId.status != 404) {
					resultFinal.listado.add(CarsService.Publicar_Anuncio(
					...
					resultFinal.listado.add("404 - Car with carID: ${resultArray.tech.mxpid} not_found")
				}
			}
		}
